...
func seedMainItem(t *testing.T, db *gorm.DB, teamID uint, oldCode string) model.MainItem {
	t.Helper()
	item := model.MainItem{TeamID: teamID, Code: oldCode, Title: "item", Priority: "P1", ProposerKey: 1}
...
func seedSubItem(t *testing.T, db *gorm.DB, teamID, mainItemID uint) model.SubItem {
	t.Helper()
	sub := model.SubItem{TeamID: teamID, MainItemKey: int64(mainItemID), Title: "sub", Priority: "P1"}
...
func TestMigrate_ValidationFailsOldFormatRemains(t *testing.T) {
	...
	require.NoError(t, db.Exec(
		"INSERT INTO pmw_main_items (team_id, code, title, priority, proposer_key, item_desc, item_status, completion, is_key_item, biz_key, create_time, db_update_time, deleted_flag, deleted_time) VALUES (9999, 'MI-0001', 'orphan', 'P1', 1, '', 'pending', 0, 0, 0, datetime('now'), datetime('now'), 0, '1970-01-01 08:00:00')",
	).Error)
...
func TestMigrate_ValidationFailsEmptySubCode(t *testing.T) {
	...
	require.NoError(t, db.Exec(
		"INSERT INTO pmw_sub_items (team_id, main_item_key, code, title, priority, item_status, completion, is_key_item, weight, biz_key, create_time, db_update_time, deleted_flag, deleted_time) VALUES (?, 9999, '', 'orphan', 'P1', 'pending', 0, 0, 1, 0, datetime('now'), datetime('now'), 0, '1970-01-01 08:00:00')",
		team.ID,
	).Error)
