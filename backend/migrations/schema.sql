-- schema.sql: Consolidated final schema (001 ~ 010)
-- Generated from all migration files. Reflects the current database state.
...
CREATE TABLE IF NOT EXISTS users (
    ...
    email           VARCHAR(100)          DEFAULT '',   -- 004
    status          VARCHAR(10)  NOT NULL DEFAULT 'enabled' -- 004
);
...
CREATE TABLE IF NOT EXISTS teams (
    ...
    code         VARCHAR(6)   NOT NULL DEFAULT '',  -- 008
    item_seq     INTEGER      NOT NULL DEFAULT 0    -- 010
);
CREATE INDEX        IF NOT EXISTS idx_teams_pm_id     ON teams(pm_id);
CREATE INDEX        IF NOT EXISTS idx_teams_deleted_at ON teams(deleted_at);
CREATE UNIQUE INDEX IF NOT EXISTS idx_teams_code      ON teams(code); -- 008
...
CREATE TABLE IF NOT EXISTS main_items (
    ...
    description       TEXT         NOT NULL DEFAULT '',  -- 005
    sub_item_seq      INTEGER      NOT NULL DEFAULT 0    -- 010
);
-- 006: global unique on code replaced by per-team unique
...
CREATE TABLE IF NOT EXISTS sub_items (
    ...
    code              VARCHAR(15)  NOT NULL DEFAULT ''  -- 008
);
CREATE UNIQUE INDEX IF NOT EXISTS idx_sub_items_main_code         ON sub_items(main_item_id, code); -- 009
...
CREATE INDEX        IF NOT EXISTS idx_sub_items_team_priority     ON sub_items(team_id, priority); -- 002
...
-- progress_records (append-only, no updated_at / deleted_at)
...
-- status_histories (append-only audit log, 007)
