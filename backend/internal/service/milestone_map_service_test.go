...
// ---------------------------------------------------------------------------
// Tests: calcOverallProgress
// ---------------------------------------------------------------------------

func TestCalcOverallProgress_EmptyMap(t *testing.T) {
	mapRepo := &mockMilestoneMapRepo{item: sampleMilestoneMap("planning")}
	msRepo := &mockMilestoneRepoForMap{milestones: []model.Milestone{}}
	miRepo := &mockMainItemRepoForMap{}
	svc := NewMilestoneMapService(mapRepo, msRepo, miRepo, nil).(*milestoneMapService)

	progress := svc.calcOverallProgress(context.Background(), 100)
	assert.Equal(t, float64(0), progress)
}

func TestCalcOverallProgress_WithItems(t *testing.T) {
	msRepo := &mockMilestoneRepoForMap{
		milestones: []model.Milestone{
			{BaseModel: model.BaseModel{BizKey: 200}},
		},
	}
	miRepo := &mockMainItemRepoForMap{
		items: []model.MainItem{
	...
	svc := NewMilestoneMapService(nil, msRepo, miRepo, nil).(*milestoneMapService)

	progress := svc.calcOverallProgress(context.Background(), 100)
	assert.Equal(t, float64(75), progress)
}

func TestCalcOverallProgress_NoItemsInMilestones(t *testing.T) {
	msRepo := &mockMilestoneRepoForMap{
		milestones: []model.Milestone{
			{BaseModel: model.BaseModel{BizKey: 200}},
		},
	}
	miRepo := &mockMainItemRepoForMap{items: []model.MainItem{}}
	svc := NewMilestoneMapService(nil, msRepo, miRepo, nil).(*milestoneMapService)
