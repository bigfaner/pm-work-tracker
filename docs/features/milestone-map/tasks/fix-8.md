---
id: "fix-8"
title: "E2E: milestone-item-management step5 drag/delete node timeout"
priority: "P0"
...
# E2E: milestone-item-management step5 drag/delete node timeout

## Root Cause

TC-MIM-S6-001 and TC-MIM-S7-001 fail with TimeoutError waiting for data-testid milestone-node to appear within 15s. Root cause: tests navigate to milestone timeline page but milestone nodes may not render within the timeout. Investigate: (1) whether the test navigates to the correct map URL, (2) whether nodes need more time to load, (3) whether the data-testid matches the actual rendered node attribute. Read MilestoneTimeline.tsx to verify node rendering. Fix selectors or add appropriate waits.

## Reference Files

- Source: tests/frontend/milestone-item-management/step5-navigate-drag.spec.ts
- Test script: 
- Test results: 
...
1. Parse `tests/frontend/milestone-item-management/step5-navigate-drag.spec.ts` to extract the first file path (comma-separated).
