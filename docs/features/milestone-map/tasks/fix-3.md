---
id: "fix-3"
title: "Fix: view_service.go compiler errors (undefined functions)"
priority: "P0"
...
# Fix: view_service.go compiler errors (undefined functions)

## Root Cause

Task 4.3 left 3 undefined function references in view_service.go: resolveMilestoneNames (L732), filterByMilestone (L735), compareMilestoneNameWithOrder (L944). The helper functions were likely added as calls but not defined. Need to add the missing function bodies.

## Reference Files

- Source: backend/internal/service/view_service.go
- Test script: cd backend && go build ./...
- Test results: undefined: resolveMilestoneNames, filterByMilestone, compareMilestoneNameWithOrder in view_service.go lines 732, 735, 944
...
1. Parse `backend/internal/service/view_service.go` to extract the first file path (comma-separated).
...
When this task is recorded as completed via `task record`, the source task 4.3 is automatically restored to pending if all its dependencies are completed.
