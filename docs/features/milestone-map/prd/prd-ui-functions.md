...
**时间线视图：**
4. 系统渲染时间线：顶部详情标题区（名称+可点击的状态 Badge，右上角编辑和删除按钮位于卡片外部），下方基本信息卡片（负责人、计划开始、计划完成、整体进度四字段同一行左右对齐，下方分隔线后显示描述，描述最多三行溢出截断，悬浮 Tooltip 展示完整内容），再下方横向时间轴，里程碑节点按计划完成时间排列，关联 MI 展示在对应里程碑下方。支持两种筛选（按名称搜索、按状态标签多选 toggle），筛选栏右侧提供重置和刷新按钮
...
- BR-5 前端守卫：里程碑图处于终态（completed/cancelled）时，"+ 创建里程碑"按钮隐藏、里程碑编辑/删除按钮隐藏、里程碑状态 Badge 不可点击
- 删除里程碑图级联说明：删除里程碑图时，系统自动软删除其下所有里程碑并解绑所有关联 MI，不受单个里程碑删除约束（BR-4）限制
...
展示单个里程碑的完整信息：名称、描述区域（第一行"描述"标签+状态 Badge+编辑按钮，第二行为描述文本，最多 6 行溢出截断，鼠标悬浮 Tooltip 展示完整内容）、计划完成时间、进度、关联 MI 列表（每行含解绑按钮）。提供编辑、删除、快速添加事项操作。

### User Interaction Flow

1. 用户在时间线上点击里程碑节点 → 右侧弹出详情面板
2. 描述区域：第一行"描述"标签（左）+ 状态 Badge（可点击切换）+ 编辑按钮（右），同行齐平；第二行为描述文本（最多 6 行溢出截断，鼠标悬浮 Tooltip 展示完整内容）。描述区域下方依次为计划完成时间和进度
...
| 进度 | decimal | 计算值 | 进度条 + 百分比 |
| 描述 | string | milestone.milestone_desc | 面板内显示，最多 6 行溢出截断，鼠标悬浮 Tooltip 展示完整内容 |
...
- 解绑：点击 MI 行右侧 × 按钮即解除绑定，显示撤销 toast；终态 MI（completed/closed）的 × 按钮隐藏（BR-3）
