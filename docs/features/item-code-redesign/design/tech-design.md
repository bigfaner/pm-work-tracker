...
| 测试文件 | 测试内容 |
|---------|---------|
| `repository/gorm/main_item_repo_test.go` | `NextCode()` 返回正确格式；序号递增；team code 快照（team code 变更后旧事项编码不变） |
| `repository/gorm/sub_item_repo_test.go` | `NextSubCode()` 返回正确格式；子序号递增 |
| `service/team_service_test.go` | `CreateTeam()` 传入 Code 字段；Code 重复返回 `ErrTeamCodeDuplicate` |
| `model/main_sub_item_test.go` | SubItem.Code 字段存在；联合唯一索引生效 |
