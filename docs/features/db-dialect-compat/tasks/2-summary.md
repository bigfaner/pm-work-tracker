...
Generate a structured summary of all completed tasks in this phase. This summary is read by subsequent tasks to maintain cross-phase consistency.

## Instructions

### Step 1: Read all phase task records

Read each record file from `docs/features/db-dialect-compat/tasks/records/` whose filename starts with `2.` and does NOT contain `.summary` (e.g., `2.1-*.md`, `2.2-*.md`, `2.3-*.md`). Exclude the phase summary's own prior record if one exists.
...
- All phase task records: `docs/features/db-dialect-compat/tasks/records/2.*.md`
- Design reference: `docs/features/db-dialect-compat/design/tech-design.md`
