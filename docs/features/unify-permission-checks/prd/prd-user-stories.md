...
## Story 3: SuperAdmin 通过权限码执行所有操作

**As a** SuperAdmin 用户
**I want to** 通过 superadmin 角色的权限码（而非硬编码短路）执行所有操作
**So that** 我的权限由 RBAC 系统统一管理，与其他角色遵循相同的校验路径

**Acceptance Criteria:**

**AC 3a — 团队管理操作:**
- Given 一个用户绑定了 superadmin 角色（superadmin 角色拥有全部 29 个权限码）
- When 该用户依次执行以下操作：创建团队（201）、更新团队信息（200）、邀请成员（200）、修改成员角色（200）、移除成员（200）、转让 PM（200）、解散团队（200）
- Then 每项操作返回括号中标注的 HTTP 状态码

**AC 3b — 事项与子事项操作:**
- Given 一个用户绑定了 superadmin 角色
...
**Acceptance Criteria:**
- Given 一个用户绑定了 superadmin 角色，且该用户在 `team_members` 表中无该团队的记录
- When 该用户发送 `GET /teams/:teamId/main-items` 访问该团队资源
- Then 返回 200，TeamScopeMiddleware 自动注入团队成员身份，不返回 403

- Given 一个普通用户（非 superadmin 角色）不是该团队成员
- When 该用户发送 `GET /teams/:teamId/main-items` 访问该团队资源
- Then 返回 403 FORBIDDEN，TeamScopeMiddleware 拒绝非成员访问（跨团队访问仅限 superadmin 角色）
...
- Then 返回包含全部 29 个权限码，所有基于权限码的 UI 元素（导航菜单、操作按钮）可见性与变更前一致
...
**As a** PM 用户
**I want to** 在 PM 身份校验改为权限码校验后，仍能正常管理团队（邀请成员、移除成员、转让 PM）
...
- When 该用户执行团队管理操作，且服务层已将 PM 身份校验替换为权限码校验
- Then 操作成功，行为与替换前一致
