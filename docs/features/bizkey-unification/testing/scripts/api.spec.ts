...
describe('API E2E Tests', () => {
  ...
  test('TC-001: Progress record stores correct snowflake team_key', async () => {
    ...
    const res = await authCurl('POST', `/api/v1/teams/${teamBizKey}/progress`, {
      body: JSON.stringify({
  ...
  test('TC-002: Invite member with PM role bizKey returns ErrCannotAssignPMRole', async () => {
    ...
    const res = await authCurl('POST', `/api/v1/teams/${teamBizKey}/members/invite`, {
      body: JSON.stringify({
        username: 'testuser',
  ...
  test('TC-003: Non-numeric teamId in URL returns 400', async () => {
    const res = await authCurl('GET', '/api/v1/teams/abc/items');
  ...
  test('TC-004: Non-positive teamId in URL returns 400', async () => {
    const resZero = await authCurl('GET', '/api/v1/teams/0/items');
    assert.equal(resZero.status, 400, `Expected 400 for teamId=0 but got ${resZero.status}: ${resZero.body}`);

    const resNeg = await authCurl('GET', '/api/v1/teams/-1/items');
  ...
  test('TC-005: Non-existent teamId returns 404', async () => {
    const res = await authCurl('GET', '/api/v1/teams/999999999999999999/items');
