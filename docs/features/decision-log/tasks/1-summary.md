...
Read each record file from `docs/features/decision-log/tasks/records/` whose filename starts with `1.` and does NOT contain `.summary` (e.g., `1.1-*.md`). Exclude the phase summary's own prior record if one exists.
...
- All phase task records: `docs/features/decision-log/tasks/records/1.*.md`
