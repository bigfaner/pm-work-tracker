...
Read each record file from `docs/features/decision-log/tasks/records/` whose filename starts with `3.` and does NOT contain `.summary` (e.g., `3.1-*.md`, `3.2-*.md`, `3.3-*.md`). Exclude the phase summary's own prior record if one exists.
...
- All phase task records: `docs/features/decision-log/tasks/records/3.*.md`
