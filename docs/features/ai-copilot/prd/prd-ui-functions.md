...
**表单生命周期**：表单卡片是消息流中的一条消息，自带状态机 —— `预填展示 → 编辑中 → 提交中 → 已提交（折叠态）/ 提交失败 / 已丢弃 / 已撤回`。**已提交、已撤回、已丢弃 三种终态默认折叠为单行摘要**（"✓ 已提交 · {title}" / "↩ 已撤回 · {title}" / "⊘ 已丢弃 · {title}"），点击可展开只读字段。**提交成功后 AI 跟进一条自然语言消息**衔接下一轮对话，避免表单成为对话末端的哑终端。表单本身的数据不二次抽取为结构化摘要——表单消息不可变即天然快照，**上下文完整性靠 transcript 自身维持**，不依赖额外的结构化 envelope。
...
- **提交成功**：表单折叠为单行"✓ 已提交 · {title}"（默认折叠态，点击展开只读字段）。**AI 跟进一条自然语言消息**（如"好，已为你创建 P1 事项「完成用户认证模块」，分配给了张三，下周五截止。还需要加里程碑吗？"）——衔接下一轮对话。可逆操作（分配 / 非 terminal 状态变更）在折叠态下方独立浮一个撤回 banner（5 分钟倒计时）；不可逆操作（terminal 状态变更）标注"该操作不可撤回"，无 banner。
...
- **撤回成功**：原已提交 turn 的折叠态不变（不改写历史），撤回作为**新的 turn** 入账，渲染为"↩ 已撤回 · {title}"折叠态。
- **丢弃**：表单折叠为"⊘ 已丢弃 · {title}"单行，保留在 transcript 中避免出现孤儿用户输入。
- **撤回窗口过期**：原已提交 turn 的折叠态 summary 不变，撤回 banner 转为"撤回窗口已过期"标注。
...
| undoDeadline | timestamp | 提交成功时间 + 5min | 到期后撤回 banner 转为"撤回窗口已过期"标注（原已提交 turn 折叠态 summary 不变） |
...
| 已提交（折叠态 · 可逆） | 表单折叠为单行"✓ 已提交 · {title}"（点击展开只读字段）+ AI 跟进自然语言消息 + 撤回 banner（mm:ss 倒计时） | API 成功 + 可逆操作（分配 / 非 terminal 状态变更） |
| 已提交（折叠态 · 不可逆） | 表单折叠为单行"✓ 已提交 · {title}"+ 标注"该操作不可撤回"+ AI 跟进消息；无撤回 banner | API 成功 + terminal 状态变更 |
| 撤回窗口过期 | 撤回 banner 转为"撤回窗口已过期"标注；折叠态 summary 不变 | undoDeadline 到达 |
| 已撤回（折叠态） | 表单折叠为单行"↩ 已撤回 · {title}"；原已提交 turn 保留（不改写历史） | 用户点撤回 + 反向操作成功（作为新 turn 入账） |
...
- **错实体防护 + 5 分钟撤回**：高影响写操作（分配/状态变更）提交前卡片必须显示目标实体的 title + bizCode 供用户二次确认。提交成功后，对**可逆操作**（分配、非 terminal 状态变更）提供 5 分钟撤回窗口；对**不可逆操作**（转入 terminal `completed`/`cancelled` 的状态变更）不提供撤回，卡片明确标注"该操作不可撤回"。
  ...
  - 撤回窗口边界：`undoDeadline` = 提交成功时间 + 5 分钟；到期后撤回 banner 转为"撤回窗口已过期"标注（原已提交 turn 折叠态 summary 不变）。
...
- **已提交/已撤回/已丢弃 form 默认折叠**：表单进入这三种终态后默认折叠为单行 summary（含状态图标 + title），点击 summary 行可展开只读字段区（含原 form 的全部字段值）；展开/折叠态记忆于会话内（同一用户再次回到该 turn 时保持上次选择）。**不抽取 keyFields/diff 等结构化摘要**——表单消息本身就是当时提交的完整快照（消息不可变），上下文完整性由 transcript 自身维持。
- **AI 跟进消息触发**：表单提交成功（committed）后，AI 必须跟发一条普通的 `ai_text` 消息（自然语言），用一两句话陈述"做了什么"+ 引导下一轮（如"还需要加里程碑吗？"）；该消息由 AI 从自己已有的上下文（form payload、用户原指令）生成，不依赖额外结构化字段或快照。失败、丢弃、撤回态不发跟进消息（错误条/折叠态 summary 已是反馈）。
- **撤回作为新 turn**：撤回作为新的 turn 入账（"↩ 已撤回 · {title}"），**不改写**原已提交 turn 的 summary 与 transcript；原已提交 turn 的撤回 banner 在撤回发生后消失（视觉上"撤回已用掉"），但原 turn 文本与折叠态保留。
...
| 已完成（成功） | 全部 ✓，下方渲染最终卡片；**自动折叠**为头部摘要（思考+计划+操作执行完成即折叠，约 600ms 延迟，让最终卡片成为视觉焦点） | 流式结束且全部成功 |
| 步骤失败 | 该步 ✗ error；**不自动折叠**（保持展开让错误可见），中断后续步骤，不渲染最终卡片改为引导文字 | 工具调用失败 |
