...
- [ ] 查询结果：摘要文字 + 内容丰富的卡片（每张展示核心字段：标题/编号/状态/负责人/截止/进度等，用户在面板内即可看必要信息；字段槽位由 [`entity-schemas.md`](./entity-schemas.md) 各实体 `result_slots` 驱动）+ 可点击跳转详情页
...
7. **决策点：是否为写操作？**
   ...
   - 否（查询）→ 返回摘要文字 + 内容丰富的卡片列表（每张展示核心字段），点击可跳转详情页。
...
flowchart TD
    ...
    Disambig --> M3
    M2 -->|否| M3{是否为写操作?}
    M3 -->|否 查询| Query[返回摘要 + 内容丰富的卡片<br/>每张展示核心字段]
