...
- [ ] 意图识别服务（4 意图：创建/查询/修改/分配 × 6 实体：MainItem/SubItem/Milestone/MilestoneMap/ProgressRecord/ItemPool，输出意图类型 + 实体字段 + 路由决策 decision：confirm/show_candidates/cannot_understand）
...
5. **决策点：意图识别路由决策（decision）？**
   - `confirm`（意图清晰）→ 进入实体解析。
   - `show_candidates`（模糊但可列候选）→ 返回引导文字 + 候选意图列表供用户选择/澄清，用户确认后进入实体解析。
   - `cannot_understand`（无意义 / 超出能力）→ 返回"无法理解，请重新描述" + 可支持的操作类型。流程结束。

   > decision 是 AI 输出的离散路由枚举，取代早期设计的浮点置信度阈值（0.7/0.4）。理由：LLM 自报置信度校准不可靠，离散决策更可验证且跨模型可移植。详见 tech-design.md §1.5 与 agent-architecture.md §3.4。
...
flowchart TD
    ...
    Stream --> M1{路由决策 decision?}
    M1 -->|confirm| Resolve[实体解析: 精确匹配 bizKey<br/>→ 模糊匹配标题]
    M1 -->|show_candidates| Clarify[返回引导文字 + 候选意图列表<br/>供用户选择/澄清]
    Clarify --> Resolve
    M1 -->|cannot_understand| Guide[返回"无法理解"<br/>列出支持的操作类型]
    Guide --> End1([结束])
    Resolve --> M2{命中多个候选?}
...
| DF003 | 外部 AI 服务 | 后端 AI 代理 | 意图类型 + 提取的实体字段 + 路由决策 decision | HTTPS | 每次用户发送 | 结构化输出 | 后端校验后才渲染卡片 |
...
- 意图识别路由决策（decision 枚举，取代浮点置信度阈值）：`confirm` 推送预填卡片；`show_candidates` 返回引导文字 + 候选意图列表供用户选择；`cannot_understand` 返回"无法理解，请重新描述"。
