...
4. 后端组装 prompt（当前 Team 实体 schema、用户权限范围、状态机规则），调用 AI 服务做意图识别 + 实体抽取。
5. **决策点：意图识别置信度 band？**
   - 置信度 ≥ 0.7（高置信）→ 进入实体解析。
...
6. **决策点：实体引用是否歧义（命中多个候选）？**
   ...
   - 否 → 继续。
7. **决策点：是否为写操作？**
   - 是（创建/修改/分配）→ 推送预填卡片。必填且无法推导的字段留空并高亮。
...
**页面导航保护：** 页面导航前若有未提交卡片，弹出确认提示"当前有未提交的操作，确定离开？"。
...
flowchart TD
    ...
    Disambig --> M3
    M2 -->|否| M3{是否为写操作?}
    M3 -->|否 查询| Query[返回摘要 + 可点击卡片]
    Query --> Jump[点击卡片跳转详情页]
    Jump --> End2([结束])
    M3 -->|是 创建/修改/分配| Card[推送预填卡片<br/>高亮必填空字段]
    ...
    PermDeny --> End1
    Input -.->|超 500 字符| Overflow[截断至 500 字符<br/>+ 提示已达长度上限]
    Overflow --> Proxy
```
...
- 输入清洗：后端对用户输入做基础清洗（移除系统指令关键词），防提示注入；单次输入最大长度 500 字符，超出截断并提示。
