...
| 进度可见性 | 主事项完成度 T+0 自动更新，延迟 < 1 秒 | 子事项追加进度后主事项完成度立即重新计算，无需 PM 手动操作 |
| 计划 vs 实际对比 | 周视图加载 < 2 秒，单页呈现计划与实际 | 无需跨文档操作，周视图直接并列展示子事项（计划）与进度记录（实际） |
| 事项闭环率 | 每个关闭事项至少有 1 条进度记录，目标闭环率 ≥ 95% | 通过状态流转强制要求：进入"待验收"前必须有进度记录 |
...
| 字段名称 | 类型 | 说明 |
|---------|------|------|
| 标题 | string | 简要描述，即本周计划内容 |
| 优先级 | enum | P1 / P2 / P3 |
| 负责人 | string | 谁来执行 |
| 完成度 | number | 最新进度记录的完成度值 |
| 状态 | enum | 见状态定义 |
| 预期完成时间 | date | 计划截止日期，超期时红色高亮 |

**排序与分页：**
- 默认排序：优先级降序（P1 → P2 → P3），同优先级按创建时间升序
- 分页：每页 20 条，支持翻页（子事项数量通常较少，可按需分页）
...
- 用户认证后方可访问系统
- 多团队隔离：用户只能访问自己所属团队的数据，跨团队数据查询在接口层拦截，超级管理员除外
