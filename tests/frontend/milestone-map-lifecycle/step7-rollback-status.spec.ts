...
test.describe('milestone-map-lifecycle / Step 7: Rollback reviewed to planning', () => {
  ...
  test.beforeAll(async ({ playwright }) => {
    ...
    const mapRes = await request.post(`/v1/teams/${teamId}/milestone-maps`, {
      headers: { Authorization: `Bearer ${authToken}` },
      data: { mapName: `e2e-s7-rollback-${runId}`, assigneeBizKey: '1' },
    });
  ...
  test('TC-MML-S7-002: Rollback status API without auth returns 401', async ({ playwright }) => {
    const request = await playwright.request.newContext({
      baseURL: 'http://127.0.0.1:8080',
      extraHTTPHeaders: { 'Content-Type': 'application/json' },
    });
