...
import { BASE, API, login, getAuthToken, parseApiData, extractBizKey } from '../helpers.js';
...
test.describe.serial('进度追加 - 自动状态流转', () => {
  ...
  test('API: pending 子事项首次追加100%直接变为 completed', async () => {
    const headers = { Authorization: `Bearer ${authToken}`, 'Content-Type': 'application/json' };

    // Create a new sub-item via API
    const subRes = await fetch(`${API}/teams/${teamId}/main-items/${testMainItemId}/sub-items`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ mainItemKey: String(testMainItemId), title: 'API自动状态-pending直接100%', priority: 'P2', assigneeKey, startDate: '2026-04-19', expectedEndDate: '2026-05-19' }),
    });
    ...
    const progressRes = await fetch(`${API}/teams/${teamId}/sub-items/${subId}/progress`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ completion: 100, achievement: '直接完成' }),
    });
    expect(progressRes.ok).toBeTruthy();

    // Fetch sub-item and verify status is completed
    const fetchRes = await fetch(`${API}/teams/${teamId}/sub-items/${subId}`, { headers });
