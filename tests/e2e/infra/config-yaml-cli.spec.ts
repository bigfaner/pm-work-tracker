...
import { runServer, writeConfig, removeConfig } from './config-setup.js';

test.describe('CLI E2E Tests — config-yaml', () => {
...
`);
      ...
      expect(result.stdout + result.stderr).toMatch(/9090|listening|started|ready/i);
    });
    ...
    test('TC-002: Use hardcoded defaults when config.yaml missing', () => {
      ...
      expect(result.stdout + result.stderr).toMatch(/8080|listening|started|ready/i);
    });
    ...
    test('TC-003: Malformed YAML causes startup failure', () => {
      ...
      expect(result.stdout + result.stderr).toMatch(/yaml|parse|syntax|config/i);
    });
  });
...
`);
      ...
      expect(result.stdout + result.stderr).toMatch(/admin|created|seed|env_admin/i);
    });
...
`);
      ...
      expect(result.stdout + result.stderr).toMatch(/9091|listening|started|ready/i);
    });
...
`);
      ...
      expect(result.stdout + result.stderr).toMatch(/9093|listening|started|ready/i);
    });
  });
...
`);
      const result = runServer();
      expect(result.exitCode).not.toBe(0);
      expect(result.stdout + result.stderr).toMatch(/jwt_secret.*32|at least 32/i);
    });
...
`);
      const result = runServer();
      expect(result.exitCode).not.toBe(0);
      expect(result.stdout + result.stderr).toMatch(/port.*1024.*65535|between 1024/i);
    });
...
`);
      const result = runServer();
      expect(result.exitCode).not.toBe(0);
      expect(result.stdout + result.stderr).toMatch(/driver.*sqlite.*mysql/i);
    });
...
`);
      const result = runServer();
      expect(result.exitCode).not.toBe(0);
      expect(result.stdout + result.stderr).toMatch(/level.*debug.*info.*warn.*error/i);
    });
...
`);
      const result = runServer();
      expect(result.exitCode).not.toBe(0);
      expect(result.stdout + result.stderr).toMatch(/format.*json.*text/i);
    });
...
`);
      const result = runServer();
      expect(result.exitCode).not.toBe(0);
      expect(result.stdout + result.stderr).toMatch(/duration|invalid.*format|unmarshal.*into/i);
    });
...
`);
      const result = runServer();
      expect(result.stdout + result.stderr).toMatch(/admin|created|seed|testadmin/i);
    });
...
`);
      ...
      expect(result.stdout + result.stderr).toMatch(/listening|started|ready|skip|exists|9094/i);
    });
...
`);
      const result = runServer();
      expect(result.stdout + result.stderr).toMatch(/listening|started|ready|9095/i);
    });
  });

  // ===== Connection Pool (TC-018 ~ TC-019) =====

  test.describe('Connection Pool', () => {
...
`);
      const result = runServer();
      expect(result.stdout + result.stderr).toMatch(/listening|started|ready|9096/i);
    });
...
`);
      const result = runServer();
      expect(result.stdout + result.stderr).toMatch(/listening|started|ready|9097/i);
    });
  });
});
