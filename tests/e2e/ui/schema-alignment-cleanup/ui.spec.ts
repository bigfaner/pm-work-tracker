...
test.describe('UI E2E Tests', () => {
  ...
  test('TC-006: TableRow.mainItemId is string type', () => {
    const matches = grepInFile(/mainItemId.*number/, projectPath('frontend', 'src', 'types', 'index.ts'));
    expect(matches.length).toBe(0);
  });

  // Traceability: TC-007 → Spec Round 4 Item 24
  test('TC-007: Shared formatDate utility extracted', () => {
    const matches = grepInDir(/function formatDate[^a-zA-Z]/, projectPath('frontend', 'src'));
    expect(matches.length).toBeGreaterThanOrEqual(1);
  });

  // Traceability: TC-008 → Spec Round 3 Item 15
  test('TC-008: Shared state recording helper extracted', () => {
    const matches = grepInDir(/useState/, projectPath('frontend', 'src'));
