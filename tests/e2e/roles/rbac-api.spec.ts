...
test.describe('RBAC — Roles (TC-039..TC-055)', () => {
  test.beforeAll(async () => {
    ...
    const teamRes = await curl('POST', `${apiUrl}/v1/teams`, {
      headers: authHeader(superadminToken),
      body: JSON.stringify({ name: `RBAC Role Team A ${runId}`, code: randomCode() }),
    });
    ...
    const team2Res = await curl('POST', `${apiUrl}/v1/teams`, {
      headers: authHeader(superadminToken),
      body: JSON.stringify({ name: `RBAC Role Team B ${runId}`, code: randomCode() }),
    });
    ...
    // 5. Create test users
    const users = [
      { username: `e2e-pm-role-${runId}`, displayName: 'Test PM Role', keyHolder: 'pm' },
      { username: `e2e-member-role-${runId}`, displayName: 'Test Member Role', keyHolder: 'member' },
      { username: `e2e-noperms-role-${runId}`, displayName: 'Test NoPerms Role', keyHolder: 'noperms' },
    ];
    ...
    // 6. Add PM and member users to team A
    const addPmRes = await curl('POST', `${apiUrl}/v1/teams/${teamBizKey}/members`, {
      headers: authHeader(superadminToken),
      body: JSON.stringify({ username: `e2e-pm-role-${runId}`, roleKey: memberRoleKey }),
    });
    ...
    const addMemberRes = await curl('POST', `${apiUrl}/v1/teams/${teamBizKey}/members`, {
      headers: authHeader(superadminToken),
      body: JSON.stringify({ username: `e2e-member-role-${runId}`, roleKey: memberRoleKey }),
    });
    expect(addMemberRes.status === 200 || addMemberRes.status === 201).toBeTruthy();

    // Transfer PM to pmUser
