...
import { findElement, screenshot, baseUrl, API, curl, login, getAuthToken, parseApiData, extractBizKey, getFirstTeamId, getFirstMemberKey } from '../helpers.js';
...
test.describe('UI E2E Tests: Status Flow Optimization', () => {
  ...
  test('TC-001: Status badge displays correct Chinese name for progressing', async ({ page }) => {
    // Get the current team ID from the page's localStorage (matches what the UI shows)
    const currentTeamId = await page.evaluate(() => {
      const raw = localStorage.getItem('team-storage');
      if (!raw) return null;
      const parsed = JSON.parse(raw);
      return parsed?.state?.currentTeamId;
    });

    // Get a token for API calls
    const token = await getAuthToken();

    // Get the first main item and transition it to "progressing"
    const itemsRes = await fetch(`${API}/teams/${currentTeamId}/main-items`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const itemsData = parseApiData(await itemsRes.json());
    const items = Array.isArray(itemsData) ? itemsData : (itemsData?.items ?? []);
    if (items.length > 0) {
      const firstItem = items[0];
      const itemId = extractBizKey(firstItem);
      if (itemId && firstItem.itemStatus !== 'progressing') {
        await fetch(`${API}/teams/${currentTeamId}/main-items/${itemId}/status`, {
          method: 'PUT',
