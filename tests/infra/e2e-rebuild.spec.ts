...
const E2E_DIR = resolve(TESTS_DIR, 'e2e');
...
test('TC-002: graduation marker and spec exist for config-yaml', () => {
  ...
  const spec = join(E2E_DIR, 'infra/config-yaml-api.spec.ts');
...
test('TC-003: graduated specs contain no stale import paths', () => {
  ...
  for (const entry of readdirSync(E2E_DIR)) {
    const dirPath = join(E2E_DIR, entry);
    if (entry.startsWith('.') || entry === 'node_modules' || entry === 'test-results') continue;
    try { const s = readdirSync(dirPath); } catch { continue; }
...
test('TC-006: playwright.config.ts exists and all specs use @playwright/test', () => {
  ...
  // Verify all spec files in tests/e2e/ import @playwright/test
  ...
  for (const entry of readdirSync(E2E_DIR)) {
    ...
    for (const file of readdirSync(dirPath).filter(f => f.endsWith('.spec.ts'))) {
      ...
      if (!content.includes('@playwright/test')) {
        missing.push(`${entry}/${file} (missing @playwright/test import)`);
      }
    }
  }
