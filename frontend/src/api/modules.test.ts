...
describe("API modules", () => {
  ...
  describe("teams", () => {
    it("createTeamApi should POST to /teams", async () => {
      ...
      expect(mockClient.post).toHaveBeenCalledWith("/teams", {
        name: "Team",
        code: "TEAM",
      });
    });
    ...
    it("updateTeamApi should PUT /teams/:bizKey", async () => {
      ...
      expect(mockClient.put).toHaveBeenCalledWith("/teams/team-bk", {
        name: "New",
      });
    });
    ...
    it("deleteTeamApi should DELETE /teams/:bizKey with body", async () => {
      ...
      expect(mockClient.delete).toHaveBeenCalledWith("/teams/team-bk", {
        data: { confirmName: "Team" },
      });
    });
    ...
    it("inviteMemberApi should POST to /teams/:bizKey/members", async () => {
      mockClient.post.mockResolvedValue(undefined);
      await teamsApi.inviteMemberApi("team-bk", {
        username: "u",
        roleKey: "member",
      });
      expect(mockClient.post).toHaveBeenCalledWith("/teams/team-bk/members", {
        username: "u",
        roleKey: "member",
      });
    });
    ...
    it("transferPmApi should PUT /teams/:bizKey/pm", async () => {
      ...
      expect(mockClient.put).toHaveBeenCalledWith("/teams/team-bk/pm", {
        newPmUserKey: "user-3",
      });
    });
  });
  ...
  describe("mainItems", () => {
    it("createMainItemApi should POST /teams/:teamBizKey/main-items", async () => {
      ...
      await mainItemsApi.createMainItemApi("team-bk", {
        title: "Item",
        priority: "P0",
        assigneeKey: "",
        startDate: "",
    ...
    it("getMainItemApi should GET /teams/:teamBizKey/main-items/:bizKey", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        "/teams/team-bk/main-items/item-bk",
      );
    });

    it("updateMainItemApi should PUT /teams/:teamBizKey/main-items/:bizKey", async () => {
      mockClient.put.mockResolvedValue({});
      await mainItemsApi.updateMainItemApi("team-bk", "item-bk", {
        title: "Updated",
      });
    ...
    it("changeMainItemStatusApi should PUT /teams/:teamBizKey/main-items/:bizKey/status", async () => {
      ...
      await mainItemsApi.changeMainItemStatusApi("team-bk", "item-bk", {
        status: "completed",
      });
      expect(mockClient.put).toHaveBeenCalledWith(
        "/teams/team-bk/main-items/item-bk/status",
        { status: "completed" },
      );
    });
    ...
    it("getMainItemTransitionsApi should GET /teams/:teamBizKey/main-items/:bizKey/available-transitions", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        "/teams/team-bk/main-items/item-bk/available-transitions",
      );
    });
  });
  ...
  describe("subItems", () => {
    it("createSubItemApi should POST to correct URL", async () => {
      mockClient.post.mockResolvedValue({});
      await subItemsApi.createSubItemApi("team-bk", "main-bk", {
        title: "Sub",
        priority: "P1",
        assigneeKey: "user-3",
        startDate: "",
      ...
      expect(mockClient.post).toHaveBeenCalledWith(
        "/teams/team-bk/main-items/main-bk/sub-items",
        {
          mainItemKey: "main-bk",
          title: "Sub",
          priority: "P1",
    ...
    it("listSubItemsApi should GET with params", async () => {
      ...
      await subItemsApi.listSubItemsApi("team-bk", "main-bk", {
        status: "open",
      });
      expect(mockClient.get).toHaveBeenCalledWith(
        "/teams/team-bk/main-items/main-bk/sub-items",
    ...
    it("getSubItemApi should GET /teams/:teamBizKey/sub-items/:bizKey", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        "/teams/team-bk/sub-items/sub-bk",
      );
    });

    it("updateSubItemApi should PUT /teams/:teamBizKey/sub-items/:bizKey", async () => {
      mockClient.put.mockResolvedValue({});
      await subItemsApi.updateSubItemApi("team-bk", "sub-bk", {
        title: "Updated",
      });
      expect(mockClient.put).toHaveBeenCalledWith(
        "/teams/team-bk/sub-items/sub-bk",
    ...
    it("changeSubItemStatusApi should PUT to status endpoint", async () => {
      mockClient.put.mockResolvedValue(undefined);
      await subItemsApi.changeSubItemStatusApi("team-bk", "sub-bk", {
        status: "done",
      });
      expect(mockClient.put).toHaveBeenCalledWith(
        "/teams/team-bk/sub-items/sub-bk/status",
        { status: "done" },
      );
    });
    ...
    it("getSubItemTransitionsApi should GET /teams/:teamBizKey/sub-items/:bizKey/available-transitions", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        "/teams/team-bk/sub-items/sub-bk/available-transitions",
      );
    });
  });
  ...
  describe("progress", () => {
    it("appendProgressApi should POST to progress endpoint", async () => {
      mockClient.post.mockResolvedValue({});
      await progressApi.appendProgressApi("team-bk", "sub-bk", {
        completion: 80,
      });
      expect(mockClient.post).toHaveBeenCalledWith(
        "/teams/team-bk/sub-items/sub-bk/progress",
        { completion: 80 },
      );
    });
  ...
  describe("itemPool", () => {
    it("submitItemPoolApi should POST to item-pool", async () => {
      ...
      expect(mockClient.post).toHaveBeenCalledWith("/teams/1/item-pool", {
        title: "Pool Item",
      });
    });
    ...
    it("listItemPoolApi should GET with params", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith("/teams/1/item-pool", {
        params: { status: "open" },
      });
    });
    ...
    it("assignItemPoolApi should POST to assign endpoint", async () => {
      ...
      await itemPoolApi.assignItemPoolApi("team-bk", "pool-bk", {
        mainItemKey: "mi-2",
        assigneeKey: "user-5",
        priority: "P2",
        startDate: "",
      ...
      expect(mockClient.post).toHaveBeenCalledWith(
        "/teams/team-bk/item-pool/pool-bk/assign",
        {
          mainItemKey: "mi-2",
          assigneeKey: "user-5",
          priority: "P2",
    ...
    it("rejectItemPoolApi should POST to reject endpoint", async () => {
      mockClient.post.mockResolvedValue({});
      await itemPoolApi.rejectItemPoolApi("team-bk", "pool-bk", {
        reason: "bad",
      });
      expect(mockClient.post).toHaveBeenCalledWith(
        "/teams/team-bk/item-pool/pool-bk/reject",
        { reason: "bad" },
      );
    });
  });
  ...
  describe("views", () => {
    it("getWeeklyViewApi should GET weekly view", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith("/teams/1/views/weekly", {
        params: { weekStart: "2026-01-06" },
      });
    });
    ...
    it("getTableViewApi should GET table view with filter", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith("/teams/1/views/table", {
        params: { priority: "P0" },
      });
    });
  ...
  describe("reports", () => {
    it("getWeeklyReportPreviewApi should GET preview", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        "/teams/1/reports/weekly/preview",
        { params: { weekStart: "2026-01-06" } },
      );
    });
    ...
    it("exportWeeklyReportApi should GET export as blob", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        "/teams/1/reports/weekly/export",
        { params: { weekStart: "2026-01-06" }, responseType: "blob" },
      );
    });
  });
  ...
  describe("admin", () => {
    ...
    it("listAdminTeamsApi should GET /admin/teams", async () => {
      ...
      expect(mockClient.get).toHaveBeenCalledWith("/admin/teams", {
