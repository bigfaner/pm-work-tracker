...
describe("UserManagementPage", () => {
  ...
  it("opens create dialog when clicking create button", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("创建用户", { selector: "[data-state] *" }),
      ).toBeInTheDocument();
    });
  });
  ...
  it("creates user and shows initial password dialog", async () => {
    ...
    const usernameInput = inputs.find(
      (el) => (el as HTMLInputElement).placeholder === "请输入账号",
    )!;
    const nameInput = inputs.find(
      (el) => (el as HTMLInputElement).placeholder === "请输入姓名",
    )!;
    ...
    await waitFor(() => {
      expect(screen.getByTestId("initial-password")).toHaveTextContent(
        "Abc123456789",
      );
    });
    expect(
      screen.getByText("请妥善保管，关闭后无法再次查看"),
    ).toBeInTheDocument();
  });
  ...
  it("opens toggle status dialog with confirmation warning", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/禁用用户后该用户将无法登录系统，但数据不会被删除/),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("reset password dialog", () => {
    ...
    it("shows validation error when password is empty on submit", async () => {
      ...
      const confirmBtn = confirmButtons.find(
        (btn) => (btn as HTMLButtonElement).closest('[role="dialog"]') !== null,
      );
      if (confirmBtn) {
    ...
    it("shows error for weak password (less than 8 chars)", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("密码需至少8位，包含字母和数字"),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("copy credentials button", () => {
    ...
    it("shows error toast on clipboard failure", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("复制失败，请手动选择文字复制"),
        ).toBeInTheDocument();
      });
    });
  });
