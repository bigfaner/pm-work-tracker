...
const seedPoolItems = [
  {
    ...
    teamKey: "1",
    title: "移动端适配需求",
    background: "当前系统在移动端浏览器下布局错位",
    expectedOutput: "响应式适配",
    submitterKey: "U001",
    poolStatus: "pending",
    ...
    dbUpdateTime: "2026-04-17T00:00:00Z",
  },
  {
    bizKey: "2",
    teamKey: "1",
    title: "用户反馈收集功能",
    background: "需要一个系统化的用户反馈收集机制",
    expectedOutput: "反馈系统",
    submitterKey: "U001",
    poolStatus: "pending",
    assignedMainKey: null,
    ...
    assignedMainCode: "",
    assignedMainTitle: "",
    createTime: "2026-04-16T00:00:00Z",
    dbUpdateTime: "2026-04-16T00:00:00Z",
  },
  {
    bizKey: "3",
    teamKey: "1",
    title: "性能优化建议",
    background: "首页加载时间超过3秒",
    expectedOutput: "加载时间降低到1秒内",
    submitterKey: "U001",
    poolStatus: "assigned",
    assignedMainKey: "1",
    assignedSubKey: "11",
    ...
    reviewerKey: "U001",
    assignedMainCode: "MI-001",
    assignedMainTitle: "",
    createTime: "2026-04-12T00:00:00Z",
  ...
  },
  {
    ...
    assignedSubKey: null,
    assigneeKey: null,
    rejectReason: "优先级不足，安排至下个季度处理",
    reviewedAt: "2026-04-10T00:00:00Z",
    reviewerKey: "U001",
    assignedMainCode: "",
    assignedMainTitle: "",
    createTime: "2026-04-08T00:00:00Z",
    dbUpdateTime: "2026-04-10T00:00:00Z",
  },
];
...
function setupHandlers() {
  server.use(
    ...
    http.post(
      "/v1/teams/:teamId/item-pool/:poolId/assign",
      async ({ request }) => {
        const body = (await request.json()) as Record<string, unknown>;
        return HttpResponse.json({
          code: 0,
    ...
    http.post(
      "/v1/teams/:teamId/item-pool/:poolId/reject",
      async ({ request }) => {
        const body = (await request.json()) as Record<string, unknown>;
        return HttpResponse.json({
          code: 0,
...
describe("ItemPoolPage", () => {
  ...
  it("shows edit button to the left of 转为主事项 when user has both permissions", async () => {
    ...
    expect(
      editBtn.compareDocumentPosition(toMainBtn) &
  ...
  it("opens submit dialog when clicking add button", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByText("新增待办事项").length).toBeGreaterThanOrEqual(
        2,
      );
    });
  });
  ...
  it("opens convert-to-main dialog when clicking button on pending item", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByText("转为主事项").length).toBeGreaterThanOrEqual(
        2,
      );
    });
  });
