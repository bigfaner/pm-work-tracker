  ...
  server.use(
    ...
    http.get("/v1/teams", ({ request }) => {
      ...
      const filtered = search
        ? seedTeams.filter(
            (t) => t.name.includes(search) || t.code.includes(search),
          )
        : seedTeams;
...
describe("TeamManagementPage", () => {
  ...
  it("displays team descriptions", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("负责 UI/UX 设计与交互体验优化"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("opens create team dialog when clicking create button", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("创建团队", { selector: "[data-state] *" }),
      ).toBeInTheDocument();
    });
  });
    ...
    server.use(
      http.get("/v1/teams", ({ request }) => {
        ...
        return HttpResponse.json({
          code: 0,
          data: {
            items: manyTeams.slice(start, start + pageSize),
