  ...
  return render(
    <MemoryRouter initialEntries={[{ pathname: initialPath, search }]}>
      <Routes>
        ...
        <Route
          path="/items"
          element={<div data-testid="items-page">Items</div>}
        />
        <Route
          path="/some-redirect"
          element={<div data-testid="redirect-page">Redirected</div>}
        />
      </Routes>
    </MemoryRouter>,
  );
...
describe("LoginPage", () => {
  ...
  it("shows inline error 账号或密码错误 on 401 response", async () => {
    const error = new Error("Unauthorized") as unknown as MockApiError;
    error.response = {
      status: 401,
  ...
  it("shows specific error for USER_DISABLED on 403 response", async () => {
    const error = new Error("Forbidden") as unknown as MockApiError;
    error.response = {
      status: 403,
    ...
    await waitFor(() => {
      expect(
        screen.getByText("账号已被禁用，请联系管理员"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows inline error on 400 validation response", async () => {
    const error = new Error("Bad Request") as unknown as MockApiError;
    error.response = {
      status: 400,
      data: {
  ...
  it("shows generic error on unexpected server error", async () => {
    const error = new Error("Server Error") as unknown as MockApiError;
    error.response = {
      status: 500,
