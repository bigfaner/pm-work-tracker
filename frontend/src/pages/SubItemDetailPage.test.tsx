...
const seedSubItem = {
  ...
  code: "SI-001-12",
  title: "Sub Alpha 2",
  itemDesc: "JWT Token implementation",
  priority: "P2",
  assigneeKey: "U003",
...
describe("SubItemDetailPage", () => {
  ...
  it("renders breadcrumb with three levels", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByText("Alpha Task").length).toBeGreaterThanOrEqual(
        1,
      );
      // "Sub Alpha 2" appears in breadcrumb and heading
      expect(screen.getAllByText("Sub Alpha 2").length).toBeGreaterThanOrEqual(
        1,
      );
    });
  });
  ...
  it("breadcrumb links are clickable", async () => {
    ...
    await waitFor(() => {
      ...
      const mainLink = alphaLinks
        .find((el) => el.closest("nav") !== null)
        ?.closest("a");
  ...
  it("opens append progress dialog", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByRole("heading", { name: "追加进度", level: 2 }),
      ).toBeInTheDocument();
    });
  });
  ...
  it("bug: shows error toast when submitting completion lower than last record", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: "追加进度" }),
      ).toBeInTheDocument();
    });
