      ...
      <ToastProvider>
        <MemoryRouter initialEntries={[`/teams/${teamId}`]}>
          <Routes>
            ...
            <Route
              path="/teams"
              element={<div data-testid="teams-page">Teams</div>}
            />
          </Routes>
        </MemoryRouter>
      </ToastProvider>
...
function setupHandlers() {
  server.use(
    ...
    http.get("/v1/admin/roles", () => {
      return HttpResponse.json({
        code: 0,
        data: { items: seedRoles, total: 4, page: 1, pageSize: 100 },
      });
    }),
    ...
    http.put("/v1/teams/:teamId/pm", async ({ request }) => {
      ...
      const newPm = seedMembers.find(
        (m) => String(m.userKey) === body.newPmUserKey,
      );
      return HttpResponse.json({
        code: 0,
        data: { newPmName: newPm?.displayName },
      });
    }),
    ...
    // Disband team
    http.delete("/v1/teams/:teamId", async ({ request }) => {
    ...
    http.post("/v1/teams/:teamId/members", async () => {
      return HttpResponse.json({ code: 0, data: null });
    }),
...
describe("TeamDetailPage", () => {
  ...
  it("displays team info card with all fields", async () => {
    renderPage();
    await waitFor(() => {
      expect(screen.getAllByText("产品研发团队").length).toBeGreaterThanOrEqual(
        1,
      );
    });
  ...
  it("invite request sends roleId instead of role string", async () => {
    ...
    await waitFor(() => {
      ...
      expect((capturedBody as Record<string, unknown>).roleKey).toBe("3");
      expect((capturedBody as Record<string, unknown>).role).toBeUndefined();
    });
  });
  ...
  it("PM row does not show change role button", async () => {
    ...
    const pmRow = rows[1];
    expect(
      within(pmRow).queryByTestId("change-role-btn"),
