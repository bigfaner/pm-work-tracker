...
import { addWeeks, formatWeekLabel, getCurrentWeekStart } from '../../utils/weekUtils'
...
export function WeekPicker({ weekStart, onChange, maxWeek, className }: WeekPickerProps) {
  ...
  return (
    ...
    >
      ...
      <span className="text-sm px-1 whitespace-nowrap">{formatWeekLabel(weekStart)}</span>
      <button
        aria-label="next week"
        disabled={isAtMax}
        onClick={() => { if (!isAtMax) onChange(addWeeks(weekStart, 1)) }}
        className={cn('flex items-center justify-center', isAtMax && 'opacity-40 cursor-not-allowed')}
      >
