...
function Breadcrumb({ className, ...props }: BreadcrumbProps) {
  return (
    <nav
      ...
      className={cn('flex items-center gap-1.5 text-[13px] text-tertiary', className)}
      {...props}
    />
  )
}
...
function BreadcrumbItem({ href, isCurrent, className, children, ...props }: BreadcrumbItemProps) {
  return (
...
function BreadcrumbSeparator({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) {
  return (
    <span className={cn('text-tertiary', className)} aria-hidden="true" {...props}>
      /
    </span>
  )
}
