...
export const handlers = [
  ...
  http.get("/v1/teams/:teamId/item-pool", ({ request }) => {
    ...
    const page: PageResult<ItemPool> = {
      items,
      total: items.length,
      page: 1,
      size: 20,
  ...
  http.post(
    ...
    async ({ params }) => {
      ...
      const resp: AssignItemPoolResp = {
        mainItemBizKey: `mi-${100 + poolId}`,
        subItemBizKey: `si-${1000 + poolId}`,
      };
      return HttpResponse.json({ code: 0, data: resp });
    },
  ),
  ...
  http.post(
    ...
    async ({ params }) => {
      const updated = makeItemPool({
        bizKey: String(params.poolId),
        poolStatus: "已拒绝",
      });
